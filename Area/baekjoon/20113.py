...
player = {k+1:0 for k in range(len(vote))}

for i in vote:
    if i != 0:
        player[i] += 1

most_vote = max(player.values())
player_vote = list(player.values())

if player_vote.count(most_vote) == 1:
    print(player_vote.index(most_vote)+1)
else:
    print("skipped")