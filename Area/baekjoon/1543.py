...
이 함수는 어떤 단어가 총 몇 번 등장하는지 세려고 한다. 그러나, 
세준이의 함수는 중복되어 세는 것은 빼고 세야 한다. 예를 들어, 문서가 abababa이고, 
그리고 찾으려는 단어가 ababa라면, 세준이의 이 함수는 이 단어를 0번부터 찾을 수 있고, 
...
while True:
    if idx >= len(word):
        break
    if idx+(idx+(len(target))) < len(word):
        if word[idx:(idx+len(target))] == target:
            idx += len(target)-1
            cnt += 1
    else:
        idx += 1
