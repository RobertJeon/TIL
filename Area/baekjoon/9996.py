...
n = int(input())
pattern = input().split("*")
pattern_len = len(str(pattern[0])+str(pattern[1]))
res = []
...
for i in range(n):
    ...
    if len(word) < pattern_len:
        ...
        if pattern[0] in word and pattern[-1] in word:
            if pattern[0] == word[:len(pattern[0])] and pattern[1] == word[-len(pattern[1]):]:
                print("DA")
            else:
                print("NE")
        else:
...
n = int(input())
pattern = input()
pattrn = pattern.split("*")
res = []

for i in range(n):
    word = input()
    if pattern[0] in word and pattern[-1] in word:
        if word.index(pattern[0]) < word.index(pattern[-1]):
            res.append("DA")
        else:
            res.append("NE")
    else:
